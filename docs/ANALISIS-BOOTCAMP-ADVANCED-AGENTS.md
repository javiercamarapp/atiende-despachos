# Análisis completo de advanced-agents → Mejoras aplicables a Atiende Despachos
...
## Patrones reutilizables (transferidos a Atiende Despachos)

### 1. APLICADO — Validation Gates entre etapas (04)
El pipeline de Atiende Despachos ahora detiene la auto-registración ERP si hay pólizas desbalanceadas o CFDIs pendientes de revisión humana. `pipeline.py` — gate antes de `register_batch`.
...
El bootcamp cae a reglas cuando el LLM falla (fail-open). Atiende Despachos lo hace mejor: timeout del LLM → `nivel="alerta"` (fail-closed) para evitar falsos negativos fiscales.

### 5. POR APLICAR — Reasoning trail persistente (02/03) → auditoría fiscal
El bootcamp mantiene `reasoning_history`/`scratchpad` en memoria. Atiende Despachos puede persistir el razonamiento del agente en el `audit_log` para auditoría fiscal completa.
...
## Lecciones de ingeniería del bootcamp (aplicables a Atiende Despachos)

1. **Scratchpad privado**: el razonamiento interno NO debe filtrarse al usuario final — solo la conclusión. (Atiende Despachos ya lo hace con `_llm_log`)
...
## 3 mejoras concretas que aplico ahora a Atiende Despachos
