# Auditoría Frontend — Atiende Despachos
...
|----------|--------|-------|
| CSS carga correctamente | Sí | Inline, no depende de archivos externos |
| Video carga | Advertencia | Depende de `/assets/hero-ai-dashboard.mp4` existir |
| Formulario envía leads | No | Solo muestra alert(), no envía nada |
| Responsive mobile | Advertencia | Landing OK, portal Jinja pierde nav en mobile |
| Accesibilidad ARIA | Advertencia | Parcial: nav toggle tiene aria-label, tablas y forms incompletos |
| Login/logout funciona | Sí | Flujo completo con bcrypt + token |
| Session management | Advertencia | Token en localStorage, 30 días sin rotación |
| CSRF protection | No | No implementado |
| Dashboard endpoints correctos | Sí | Modelos Pydantic validados, multi-tenant filtering |
| Charts/gráficas | Sí | Chart.js con bar + doughnut charts |
| Assets optimizados | Advertencia | CSS inline OK, pero CDN sin SRI |
| Lazy loading | Sí | YouTube iframe tiene loading="lazy" |
| Meta tags SEO | No | Solo description, faltan OG/Twitter/JSON-LD |
| Manifest.json válido | Sí | Válido, pero start_url requiere auth |
| Service worker | Sí | Implementado con cache-first + network-first API |
| Validación client-side | Sí | HTML5 required + type=email en formularios |
| SPA routing | No | Sin History API, sin deep links |
| 404 handling | No | No hay página 404 |
| Keyboard navigation | Advertencia | Básica (tab order natural), sin skip-links |
| Screen reader | Advertencia | Parcial, faltan labels en tablas y forms |
