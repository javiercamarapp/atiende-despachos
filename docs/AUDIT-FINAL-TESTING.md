...
|---|---|---|
| Cobertura de módulos | MEDIA | 69 módulos sin referencia alguna en tests |
| Calidad de assertions | MEDIA | 769 assertions solo verifican status_code; muchos tests smoke |
...
| Concurrency | BAJA | Solo 3 tests de concurrencia (threading), sin multi-tenant race conditions |
| Performance | MÍNIMA | 1 test de "performance" (test_analytics.py:1029), sin benchmarks reales |
| Snapshot XML | BAJA | Solo 2 snapshot tests en test_enterprise_hardening.py |
