#!/usr/bin/env bash
# =============================================================================
# scripts/deploy-railway.sh — Deploy one-click de Atiende Despachos a Railway
#
# Uso:
...
deploy() {
  ...
  while [ "$waited" -lt "$HEALTH_TIMEOUT" ]; do
    if curl -fsS -m 5 "${url}/health" >/dev/null 2>&1; then
      ...
      echo ""
 echo " Atiende Despachos desplegado:"
      echo "    API:  $url"
      echo "    Docs: $url/docs"
...
setup_domain()  { local domain="${1:-api.atiende-despachos.app}"; railway domain "$domain"; }
teardown() {
...
Opciones:
  ...
  --domain D   Asignar dominio custom (default: api.atiende-despachos.app)
...
case "${1:-}" in
  ...
  --logs)     view_logs ;;
  --status)   view_status ;;
  --domain)   setup_domain "${2:-api.atiende-despachos.app}" ;;
  --teardown) teardown ;;
  -h|--help)  usage ;;
